...
Стек: **Next.js 16 + Tailwind v4 + TypeScript** на проде,
**shadcn/ui — по требованию** (когда впервые понадобится сложный
компонент), **n8n + Gemini API** для AI-консьержей.
...
- **hotel-pitch-template** — отдельный шаблон для превью-сайтов
  (cold/warm заходы, пресейл), живёт в
  `/home/balance/projects/hotel-pitch-template/` (на одном уровне
  с этим проектом) как независимый git-репозиторий:
  https://github.com/Balance134/hotel-pitch-template.
  Используется для пресейла, **не для прода**.

  `web-base-template` (этот) — для production-сайтов после
  оплаты клиента. Разворачивание дорогое по времени и токенам,
  для холодных заходов не подходит.
