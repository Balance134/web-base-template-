...
- **hotel-pitch-template** — отдельный шаблон для превью-сайтов
  (cold/warm заходы, пресейл), живёт в соседней папке
  `../hotel-pitch-template/` как независимый git-репозиторий.
  Используется для пресейла, **не для прода**. Ссылка на репо
  появится после его создания.
